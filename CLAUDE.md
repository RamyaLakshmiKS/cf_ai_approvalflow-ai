...
- **AI SDK Tools** (`src/ai-tools.ts`): Zod-based schemas for the AI SDK's `tool()` function
- **Custom Tools** (`src/tools.ts`): Manual tool definitions with typed parameters

All tools require a `ToolContext` containing `{ env: Env, userId: string }`
...
- `submit_expense_request` - Create expense reimbursement request

**4. Frontend (`src/app.tsx` and `src/client.tsx`)**

- React-based chat UI with Tailwind CSS
- Uses `useAgent` hook from `agents/react` for WebSocket connections
- Components follow shadcn/ui patterns
- Theme switching (light/dark mode) via `useTheme` hook
...
- Demo users seeded via migration `migrations/0007_seed_ramya_users.sql`
...
- Company policies defined in `docs/employee_handbook.md`
...
2. Create AI SDK wrapper in `src/ai-tools.ts`:
...
export function createAITools(context: ToolContext) {
  return {
    ...
    my_tool: tool({
      description: "What it does",
      parameters: z.object({...}),
      execute: async (params) => await customTools.my_tool.execute(params, context)
    })
  };
}
```

3. Update system prompt in `src/prompts.ts` to describe when/how to use the tool
...
- Keep code in single files unless specified otherwise
- Follow Cloudflare Workers security best practices (no secrets in code)
- Use Durable Object WebSocket Hibernation API (not legacy addEventListener)
- For Agents: use `this.setState()` for state management, `this.sql` for direct database access
- Set `new_sqlite_classes` in wrangler.jsonc for Agent persistence
